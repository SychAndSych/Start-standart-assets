...
{
    [AddComponentMenu("Sych scripts / Game / Base / Ragdoll activity")]
    ...
    public class Ragdoll_activity : MonoBehaviour
    {
        ...
        bool Enable_Projection_and_Preprocessing_bool = false;


        [Space(20)]
        [Header("HurtBox")]

        [Tooltip("Основной скрипт здоровья для назначения")]
        [SerializeField]
        Health Health_script = null;

        [Tooltip("Слой который будет применим для HurtBox")]
        [SerializeField]
        int HurtBox_layer = 8;

        [field: Tooltip("Режим когда части куклы активны для получения урона")]
        [field: SerializeField]
        internal bool HurtBox_mode_bool { get; private set; } = false;//Теперь части куклы считаются HurtBox'сами
        ...
        /// <summary>
        /// Подготовка для работы с параметрами костей
        /// </summary>
        void Preparation_bones()
        {
        ...
        /// <summary>
        /// Корутина на автоматическое вставание
        /// </summary>
        /// <returns></returns>
        ...
        public void Active_change(bool _active)
        {
            ...
            else if (Active_bool && !Get_up_bool && !_active)
            {
                Up_ragdoll();
            }
        ...
        /// </summary>
        /// <param name="_active">Активность</param>
        void Activity_base_collider_and_rigibody(bool _active)
        {
        ...
        void Activity_ragdoll(bool _active)
        {
            ...
            {
                if (HurtBox_mode_bool)
                {
                    collider.isTrigger = !_active;
                }
                else
                {
                    collider.enabled = _active;
        ...
        /// <summary>
        /// Автоматически устанавливает скрипт передающий получаемый урон от частей куклы в скрипт здоровья
        /// </summary>
        public void Auto_add_HurtBox_component()
        {
            if (Health_script)
            {
                for (int x = 0; x < Collider_array.Length; x++)
                {
                    if (Collider_array[x].GetComponent<HurtBox>())
                    {
                        Collider_array[x].GetComponent<HurtBox>().Add_Main_health(Health_script);
                    }
                    else
                    {
                    ...
                    Collider_array[x].enabled = true;
                    Collider_array[x].isTrigger = false;
            ...
            {
                Debug.LogError("Не указан основной Health скрипт");
            }
        ...
        /// </summary>
        public void Auto_Find_components()
        {
            if (!Anim)
            {
        ...
        /// <summary>
        /// Включает или выключает у джоинтов куклы Projection и Preprocessing
        /// </summary>
        public void Enable_Projection_and_Preprocessing()
        {
            Enable_Projection_and_Preprocessing_bool = !Enable_Projection_and_Preprocessing_bool;

            for (int x = 0; x < Rigidbody_array.Length; x++)
            {
                if (Rigidbody_array[x].GetComponent<CharacterJoint>())
                {
                    Rigidbody_array[x].GetComponent<CharacterJoint>().enableProjection = Enable_Projection_and_Preprocessing_bool;
                    Rigidbody_array[x].GetComponent<CharacterJoint>().enablePreprocessing = Enable_Projection_and_Preprocessing_bool;
                }
