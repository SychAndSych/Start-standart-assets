    ...
    public class Input_setting : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            if (Setting_menu.Singleton_Instance)
                Setting_menu.Singleton_Instance.Reset_d += Reset_button;
        }
                    ...
                    foreach (KeyCode _key in KeyCode.GetValues(typeof(KeyCode)))
                    {
                        if (Input.GetKeyDown(_key))
                        {
                            ...
                            if (FindObjectOfType<Setting_menu>())
                                Setting_menu.Singleton_Instance.Input_key_control();//Высылает всем заинтересованным в изменение управления, через другой скрипт
